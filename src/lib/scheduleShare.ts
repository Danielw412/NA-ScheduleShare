...
export const scheduleShareTitle = 'Schedule | NA ScheduleShare'

const shareServiceBaseUrl = import.meta.env.VITE_SCHEDULE_SHARE_BASE_URL?.trim().replace(/\/$/, '')
const sharePreviewVersion = '2'
...
export async function createScheduleShareUrl(): Promise<string> {
  ...
  return `${shareServiceBaseUrl}/share/${encodeURIComponent(token)}?v=${sharePreviewVersion}`
}
