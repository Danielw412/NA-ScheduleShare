...
describe('MeetingSlotGrid', () => {
  ...
  it('keeps the full labeled grid horizontally scrollable on mobile', () => {
    ...
    expect(styles).toMatch(/\.meeting-slot-grid-scroll\s*\{[^}]*min-width\s*:\s*0\s*;[^}]*overflow-x\s*:\s*auto\s*;/)
    expect(styles).toMatch(/\.meeting-slot-grid\s*\{[^}]*width\s*:\s*100%\s*;[^}]*min-width\s*:\s*520px\s*;/)
  })
})
