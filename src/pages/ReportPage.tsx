  ...
  return (
    <div className="report-page narrow-page">
      ...
      <form className="stacked-form" onSubmit={(event) => void submit(event)}>
        ...
        {targetType === 'class' ? <div className="report-target-field"><span>Class</span>{selectedClass ? <div className="selected-report-target"><span><strong>{selectedClass.course_name}</strong><small>{selectedClass.teacher_last_name}</small></span><button type="button" aria-label="Choose a different class" onClick={() => { setSelectedClass(null); setClassQuery('') }}><X size={17} /></button></div> : <><label className="search-input"><Search aria-hidden="true" /><span className="sr-only">Search classes by course or teacher</span><input value={classQuery} onChange={(event) => { setClassQuery(event.target.value); setError(null) }} placeholder="Search course or teacher last name" autoComplete="off" /></label><div className="report-target-results" aria-live="polite">{searching ? <p className="muted">Searching…</p> : classQuery.trim().length < 2 ? <p className="muted">Enter at least two letters of the course or teacher name.</p> : classResults.length === 0 ? <p className="muted">No classes match that search.</p> : classResults.map((course) => <button type="button" key={course.id} onClick={() => { setSelectedClass(course); setClassQuery(''); setClassResults([]) }}><strong>{course.course_name}</strong><small>{course.teacher_last_name} · {course.meeting_slots.map((slot) => `${slot.day_type} P${slot.period_number}`).join(', ')}</small></button>)}</div></>}</div> : null}
