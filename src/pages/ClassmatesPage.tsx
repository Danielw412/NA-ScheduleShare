  ...
  return (
    <DiscoveryGate>
      <div className="classmates-page">
        ...
        <div className="classmate-list">{classmates.map((classmate) => <article key={classmate.student_id}><ProfileAvatar userId={classmate.student_id} fullName={classmate.full_name} /><div><h2>{classmate.full_name}</h2><p>Grade {classmate.grade}</p><div>{classmate.shared_course_names.map((name) => <span className="shared-class" key={name}>{name}</span>)}</div></div>{classmate.can_view_schedule ? <Link className="button button-secondary" to={`/students/${classmate.student_id}`}>View full schedule</Link> : <span className="private-label"><Users size={16} /> Shared classes only</span>}</article>)}</div>
      </div>
    </DiscoveryGate>
  )
}
