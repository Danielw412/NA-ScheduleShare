-- Remove both semester lunch enrollments when they represent one full-year
-- lunch choice at the same period. Semester-specific lunches at different
-- periods remain independent.
...
begin
  ...
  select class_record.course_name_id, course_name.term_policy
  into target_course_name_id, target_policy
  ...
  if target_policy = 'lunch'
    ...
    if target_period is not null then
      ...
      where candidate.student_id = actor_id
        ...
        and candidate_class.course_name_id = target_course_name_id
...
comment on function public.remove_enrollment(uuid) is
  'Removes one enrollment. Matching Semester 1 and Semester 2 lunch enrollments are removed together only when they use the same course and period.';
